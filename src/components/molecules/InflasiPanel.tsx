...
const InflasiPanel: React.FC = () => {
  ...
  const renderGrid = (prefix: string, title: string) => (
    <div className="mb-8">
      <h3 className="font-bold mb-3 italic text-xl text-kemenkeublue">{title}</h3>
  ...
  return (
    <div className="rounded-xl shadow-md overflow-hidden">
      <div className="p-5 bg-white space-y-10">
        {renderGrid("yoy", "Inflasi year on year (yoy)")}
        <hr className="border-t-2 border-dashed border-grey-800 my-4" />
        {renderGrid("mtm", "Inflasi month to month (mtm)")}
      </div>
    </div>
  );
};

export default InflasiPanel;
