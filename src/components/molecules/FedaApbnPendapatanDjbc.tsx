...
interface FedaApbnPendapatanDjbcProps {
  tahun: string;
  bulan: string;
  provinsi: string;
  instansi: string;
...
const FedaApbnPendapatanDjbc: React.FC<FedaApbnPendapatanDjbcProps> = ({
  tahun,
  bulan,
  provinsi,
  instansi,
      ...
      <div className="grid grid-cols-3 gap-8">
        ...
        <div className="p-4 rounded-xl bg-kemenkeubluesoft/30 border-2 border-dashed border-gray-400">
          ...
          {posList.map((pos) => {
            ...
            if (value !== "-") {
              const numeric = parseFloat(value.replace(/\./g, "").replace(",", ".").replace("%", ""));
...
export default FedaApbnPendapatanDjbc;
