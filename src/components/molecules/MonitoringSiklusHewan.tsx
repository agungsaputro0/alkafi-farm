...
import riwayatSiklusHewanTernakData from "../pseudo_db/monitoringsiklushewan.json";
...
import MonitoringCycleCard from "../atoms/MonitoringCycleCard";
...
const MonitoringSiklusHewan = () => {
  ...
  const localStatus: StatusHewanLocal[] = JSON.parse(
    localStorage.getItem("MonitoringSiklusHewanBaru") || "[]"
  );
  ...
  const getStatus = (id: string) =>
    riwayatSiklusHewanTernakData.find((r) => r.idHewanTernak === id)?.jenisCatatan || "-";
  ...
  const mergedData = data.map((item) => {
    ...
    const riwayatHewan = riwayatSiklusHewanTernakData
      .filter((r) => r.idHewanTernak === item.idHewanTernak)
      .sort((a, b) => new Date(b.tanggalPencatatan).getTime() - new Date(a.tanggalPencatatan).getTime());
    const terakhir = riwayatHewan[0];

    return {
      ...item,
      jenisCatatan: local ? local.jenisCatatan : terakhir?.jenisCatatan || "-",
      detail: local?.detail || terakhir?.detail || "-",
      tanggalPencatatan: local?.tanggalPencatatan || terakhir?.tanggalPencatatan || "-",
    };
  });
  ...
  const filteredData = mergedData.filter((item) => {
    ...
    const status = getStatus(item.jenisCatatan).toLowerCase();
      ...
      <MainPanel>
        <div className="px-6">
          <p className="text-white font-semibold">
            <span onClick={() => navigate("/ternak/repository")} className="text-white/50 hover:text-kemenkeuyellow cursor-pointer">Hewan Ternak • </span> Monitoring Siklus
          </p>
        </div>
        ...
        <WhiteSection>
          <div className="p-6 bg-[#fffdf9]/90 backdrop-blur-md rounded-2xl shadow-lg border border-gray-100">
            ...
            <div className="flex items-center gap-4 pb-5 mb-6 border-b-2 border-farmLightOrange bg-gradient-to-r from-farmlighestbrown to-farmBoldPeach rounded-xl p-4 shadow-sm">
              <GiCctvCamera className="text-7xl text-farmdarkestbrown" />
              <div>
                <h2 className="text-3xl font-extrabold text-farmdarkestbrown font-spring tracking-wide drop-shadow-sm">
                  Monitoring Siklus Hewan Ternak
                </h2>
                <p className="text-sm text-[#724e3a]">
                  Pantau siklus reproduksi dan kematian setiap hewan ternak.
                </p>
              </div>
            </div>
            ...
            <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
              {paginatedData.map((item) => (
                <MonitoringCycleCard
                  ...
                  statusKesehatan={item.jenisCatatan}
...
export default MonitoringSiklusHewan;
